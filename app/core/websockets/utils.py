...
def emit_dashboard_event(payload: dict):
    try:
        loop = asyncio.get_running_loop()
        loop.create_task(manager.broadcast_dashboard(payload))
    except RuntimeError:
        asyncio.run(manager.broadcast_dashboard(payload))
...
def emit_user_event(user_id: int, payload: dict):
    try:
        ...
        asyncio.run(manager.broadcast_to_user(user_id, payload))
